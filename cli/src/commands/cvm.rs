...
use crate::{
    atls_policy_store::{self, PolicyBundle, PolicyWriteStatus},
    cli::{
...
fn launch(config: &ResolvedConfig, args: CvmLaunchArgs, json_output: bool) -> ExitStatus {
    ...
    let policy_file =
        match atls_policy_store::write(&config.config_dir, &result.policy_bundle, &result.cvm.id) {
            Ok(value) => value,
            Err(message) => {
                style::eprintln_error(&message);
...
fn update(config: &ResolvedConfig, args: CvmUpdateArgs, json_output: bool) -> ExitStatus {
    ...
    let (policy_file, policy_status) = match atls_policy_store::write_after_update(
        &config.config_dir,
...
fn selected_launch_profiles(
    ...
    let profiles = match config.profiles.clone() {
        named if !named.is_empty() => named,
        _ => auto_select_profile(console_url, session)?,
    };
...
fn persist_launch_defaults(
    ...
    if config.profiles.is_empty() && profile_ids.len() == 1 {
