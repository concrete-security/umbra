...
def keep_failed_cvm_resources() -> bool:
    ...
    UMBRA_KEEP_FAILED_CVM=1 to leave the Phala app and DNS records in place so the
    in-CVM logs (e.g. nginx-cert-manager / ACME cert issuance) survive for inspection.
    Re-enable teardown by unsetting the flag.
    """
    raw = load_settings().raw.get("UMBRA_KEEP_FAILED_CVM", "").strip().lower()
...
def security_cvm_update_material() -> dict[str, str]:
    ...
    from umbra_console.routes import render_security_cvm_compose_config
