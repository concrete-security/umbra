...
from umbra_console.audit import AUDIT_ACTIONS, insert_audit_event, redact_user_audit_trail
...
from umbra_console.idempotency import (
...
def require_launchable_instance_type(instance_type: str) -> None:
    ...
    raise api_error(
        ...
        message,
        {
            ...
            "hint": "relaunch with a valid --instance-type; run `umbra cvm instance-types` for specs",
        },
    )
...
def cvm_list_state_clauses(
    ...
    *,
    ...
    - an explicit state: keep ``deleted_at`` and match ``c.state = $N`` (uppercased).
    """
    ...
    # An explicit state: keep live rows and match exactly that state.
    return (
...
async def apply_provider_cvm_lifecycle_action(*, app_id: str, action: str) -> None:
    from umbra_console.tee_provider import CvmProviderError, cvm_provider_from_settings
...
async def deprovision_security_cvm_dns_records(row: asyncpg.Record | dict) -> set[str]:
    from umbra_console.dns_provider.cloudflare import CloudflareClient, CloudflareError
