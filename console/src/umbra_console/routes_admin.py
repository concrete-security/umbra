...
from umbra_console.admin_log_buffer import (
...
from umbra_console.readiness import run_ready_checks
...
from umbra_console.scheduler import scheduler_last_tick_age_seconds
from umbra_console.tee_provider.phala import PhalaClient, PhalaError, managed_cvm_name
...
async def _fetch_phala_status_by_name() -> dict[str, dict[str, Any]]:
    ...
    for row in rows:
        name = managed_cvm_name(row) or row.get("name") or row.get("cvm")
...
def parse_metrics_summary() -> dict[str, Any]:
    ...
    for line in text.splitlines():
        if line.startswith("umbra_console_requests_total{"):
        ...
        if line.startswith("umbra_console_redacted_value_in_log_total"):
            parts = line.rsplit(" ", 1)
            if len(parts) == 2:
                try:
