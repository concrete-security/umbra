...
const nextConfig = {
  ...
  async headers() {
    ...
    // - In prod we disallow inline/eval and only allow HTTPS/WSS connects
    ...
    const prodCsp = "default-src 'self'; script-src 'self'; connect-src 'self' https://*.supabase.co wss://*.supabase.co https://*.phala.network wss://*.phala.network; img-src 'self' blob: data:; style-src 'self' 'unsafe-inline'; font-src 'self'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'; object-src 'none'; upgrade-insecure-requests";
