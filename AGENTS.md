...
- The shade checkout at `SHADE_DIR` is part of the deployed artifact and is pinned by `SHADE_REF` in `.env.common`. `make deploy` converges it; the verify preflight asserts it. Never hand-edit it on a host. Bump `SHADE_REF` in the same change as any Console edit that alters the shade contract.
