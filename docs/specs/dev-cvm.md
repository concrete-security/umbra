...
| `SECURITY_CVM_ATLS_POLICY_B64` | `dev-egress-forwarder:/run/umbra/security-cvm.atls-policy.json` | `0444` | Console-supplied atlas/shade aTLS policy for the entity SC, stored at `security_cvms.metadata.atls_policy` during SC provisioning (`docs/specs/console.md` §7.11, §8.4 step 3) | Verified by `dev-egress-forwarder` against the SC's TDX attestation before forwarding traffic (§4.5). This value MUST NOT be injected into `user-sandbox`. |
...
| `DEV_CVM_CONTROL_TOKEN` | n/a | n/a | Console-minted per-Dev-CVM `service_principal_tokens` plaintext (`principal_type=dev_cvm`, `purpose=DEV_CONTROL`) | Bearer the `dev-egress-forwarder` presents only to Console Dev-control routes, such as SC aTLS policy refresh (§4.5). The Security CVM MUST NOT accept this token for proxy traffic. This value MUST NOT be injected into `user-sandbox`. |
| `CONSOLE_URL` | n/a | n/a | Console origin URL | Used only by `dev-egress-forwarder` to fetch a refreshed Security CVM aTLS policy after local SC attestation fails. It is RTMR3-bound so a deploy-plane redirect to a hostile control plane is detected at Dev CVM tunnel verification. The forwarder uses this recovery path only for HTTPS origins; non-HTTPS local/dev origins disable refresh instead of weakening transport. |
...
- **Runtime material is delivered at the boundary and bound by attestation.** SSH keys, the SC mitmproxy CA, the SC aTLS policy, and the per-CVM proxy/control bearers all arrive via Phala's env-file at deploy time (§2.3, §6.1, `docs/specs/console.md` §8.3 step 3). Each value is injected only into the service that needs it: `user-sandbox` receives the CA, authorized keys, and non-secret sandbox placeholders; `dev-egress-forwarder` receives the CA, aTLS policy, proxy bearer, and Dev-control bearer. The service entrypoints materialise their values into `/run/umbra/*` under restrictive modes before serving traffic (§10). RTMR3 is extended at boot with the SHA-256 digests of these values (`docs/specs/console.md` §10.4a) so any deploy-plane swap is detected by the CLI's `atlas-rs` policy verification at first tunnel (§9). The dev user can read the sandbox's CA/keys/placeholders inside their namespace, but cannot read the forwarder's bearers or aTLS policy through the normal compose boundary, cannot rewrite the RTMR digest, and cannot exfiltrate material past the topology-bound egress.
...
- Verifies the Security CVM endpoint against the aTLS policy at `/run/umbra/security-cvm.atls-policy.json` (materialised at boot from the env-file value `SECURITY_CVM_ATLS_POLICY_B64`; see §2.3, §10). Fails closed if the policy is missing, the endpoint cannot be reached, or attestation does not satisfy the policy.
...
- If that local policy fails verification, the forwarder MAY call `GET ${CONSOLE_URL}/internal/dev-control/security-cvm-atls-policy` with its existing `DEV_CVM_CONTROL_TOKEN` bearer to fetch a refreshed candidate SC aTLS policy. This refresh is for SC application-image or full-runtime-policy changes; an app-image release does not change the shared guest MRTD. The forwarder MUST reject the candidate if `security_cvm_fqdn` differs from the launch-bound `SECURITY_CVM_FQDN`, if `ca_cert_sha256` differs from the latest SC CA accepted through the authenticated runtime CA-refresh path below, or if the candidate policy disables runtime verification or omits `app_compose`, `expected_bootchain`, or `os_image_hash`. The Console response is a candidate delivery channel, not a substitute for local aTLS verification. The SC `PROXY_AUTH` bearer MUST NOT be sent to Console for this refresh and the `DEV_CVM_CONTROL_TOKEN` MUST NOT be accepted by the Security CVM proxy. A CA digest mismatch rejects that candidate and the request fails closed until the independent CA poll accepts and publishes the current CA; on a refresh-capable Umbra runtime it does not require `cvm.update`. **Trust caveat:** this v0 path still relies on the Console to materialize the candidate SC runtime policy; making the Console only an untrusted cache requires release-pipeline-signed SC policy material that the forwarder/helper verifies before accepting the candidate.
...
2. **Materialise measured runtime values from env-file.** Require the non-secret `SECURITY_CVM_FQDN`. Decode `SECURITY_CVM_CA_CERT_B64` to `/run/umbra/security-cvm-ca.launch.pem` and `AUTHORIZED_SSH_KEYS_B64` to its documented path/mode. Decode `SANDBOX_ENV_PLACEHOLDERS_B64` to `/run/umbra/sandbox-env-placeholders` when present; an empty placeholder file is valid. `SECURITY_CVM_ATLS_POLICY_B64` and `SECURITY_CVM_PROXY_TOKEN` are forwarder-only values and MUST NOT be present in `user-sandbox`. Refuse boot if any required sandbox value is missing or empty. The base64 env vars SHOULD be unset after materialisation; `SECURITY_CVM_FQDN` remains for the CA watcher.
...
- **No forwarder-only runtime secrets in `user-sandbox`.** `SECURITY_CVM_PROXY_TOKEN`, `/run/umbra/proxy-token`, and `SECURITY_CVM_ATLS_POLICY_B64` MUST exist only in `dev-egress-forwarder`. The sandbox receives the SC CA so HTTPS clients can trust SC-intercepted traffic, but it does not need the bearer or the SC aTLS policy to run shells, agents, Docker, or editor sessions.
