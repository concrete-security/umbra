...
Authoritative bootstrap and refresh endpoint for a Dev CVM's `dev-egress-forwarder`. The forwarder MAY listen without a policy for measurement compatibility, but before its first upstream SC connection or `200 Connection Established` it MUST fetch the complete stored SC aTLS policy here with its Dev-control bearer, strictly validate it, and atomically install it. A bounded fetch or validation failure produces fail-closed `502` and no upstream connection; a blank, stub, dev, disabled, or incomplete policy and every bypass are forbidden. It periodically refreshes from this same endpoint afterward. The full policy MUST NOT be placed in Phala's Dev-CVM launch env. This route MUST NOT return a new CA, a new Dev-CVM proxy bearer, profile policy, or user-facing secret.
...
- **Trust boundary.** The Console's full SC policy generation, storage, and Console-side SC attestation remain unchanged. This route transports that complete stored policy instead of the provider launch env; the Dev forwarder/helper still rejects malformed, disabled, or incomplete input and performs local aTLS verification before forwarding. The current temporary helper behavior validates the delivered policy first and only then strips image/runtime pins as documented in `docs/sc-policy-check-disabled.md`; the control plane MUST NOT send a policy with `disable_runtime_verification` set. This route does not, by itself, make a Console-authored golden measurement independently trustworthy: v0 relies on the Console to materialize the candidate SC measurement, then narrows acceptance to the launch-bound SC FQDN and the latest CA accepted through the authenticated runtime CA endpoint. To make the Console only an untrusted cache, production deployments need release-pipeline signed SC measurement material embedded in or referenced by `atls_policy`, with the Dev forwarder/helper rejecting unsigned or unverifiable measurement updates.
...
| 3. `phala_deploy` | Step 2 done; the Dev CVM bearers minted in step 4 are computed first so the deploy env can carry them (saga step ordering MAY interleave the local-DB step 4 mint before the external-call step 3 in the implementation; the table lists them in their logical commit order). Require a complete stored `security_cvms.metadata.atls_policy`; if absent, fail closed before deployment with `SECURITY_CVM_ATLS_POLICY_UNAVAILABLE`, because the new forwarder could not bootstrap. Build the env dict locally — `SECURITY_CVM_FQDN`, `SECURITY_CVM_PROXY_PORT`, `SECURITY_CVM_PROXY_TOKEN` (the plaintext SC proxy bearer from step 4), `DEV_CVM_CONTROL_TOKEN` (the plaintext Console Dev-control bearer from step 4), `SECURITY_CVM_CA_CERT_B64`, `AUTHORIZED_SSH_KEYS_B64`, `SANDBOX_ENV_PLACEHOLDERS_B64` — and pass it to Phala's deploy via the env-file mechanism. The full SC policy MUST NOT be included in this env; before its first upstream SC connection or successful CONNECT response, the forwarder fetches it from `GET /internal/dev-control/security-cvm-atls-policy`, and it periodically refreshes from that endpoint thereafter. The authoritative inventory lives in `docs/specs/dev-cvm.md` §2.3; the Console MUST inject every launch-bound value the Dev CVM consumes, including the values whose SHA-256 digests appear in the RTMR3 binding (§10.4a). **The env dict is NOT persisted on the Console side** (§7.9): it would carry plaintext secrets and serves no spec-level reader. If `cvms.metadata->>'app_id'` is set (re-entry), call `phala.get_status` instead. Else call `phala.deploy`. Write `cvms.metadata = {"provider": "phala", "app_id": <id>, "gateway_host": <host>, "policy_bundle": <PolicyBundle>}`. The FQDN is the certificate, attestation, TCP target, TLS SNI, and HTTP Host identity. | `cvms.metadata->>'app_id'` is non-null. | Compensate steps 3..1; mark operation `failed`. The env dict (held in saga-local memory only) is zeroed before the saga returns. |
...
8. **Security CVM attestation refresh** (§10.4, T-29, T-30). For every `security_cvms` row in state `RUNNING` whose `attestation_verified_at < now - RECONCILER_ATTESTATION_INTERVAL_SECONDS` (default `21600`, §12), open an aTLS connection to the SC's `/tdx_quote` endpoint, regenerate the complete Shade policy from the deployed compose, and run atlas-rs verification over `app_compose`, `expected_bootchain` (including the row's shared guest MRTD), `os_image_hash`, and the RTMR JCS replay. On success update `attestation_verified_at` and `image_measurement` / `rtmr3_digest` if not already persisted; emit `SECURITY_CVM_ATTESTATION_VERIFIED`. On any runtime-policy mismatch, set `error_reason = "ATTESTATION_DRIFT"`, emit `SECURITY_CVM_ATTESTATION_DRIFT`, and page the operator (§17.4) — the row is NOT auto-decommissioned. A refresh attempt failing with the connectivity code (`ATTESTATION_FETCH_FAILED`) is tolerated as transient (logged at `WARN`, retried next pass) while the last successful attestation is younger than `RECONCILER_ATTESTATION_INTERVAL_SECONDS + RECONCILER_ATTESTATION_UNREACHABLE_GRACE_SECONDS` (§12); past that, set `error_reason = "ATTESTATION_UNREACHABLE"` (never overwriting an existing `ATTESTATION_DRIFT` verdict or `SECURITY_CVM_REBIND_REQUIRED` marker, and only if `attestation_verified_at` still matches the claimed snapshot) and emit `SECURITY_CVM_ATTESTATION_UNREACHABLE` once per outage — the next successful refresh clears the mark.
9. **Dev CVM attestation refresh** (§10.4a, T-31, T-32). For every `cvms` row in state `RUNNING` whose `attestation_verified_at < now - RECONCILER_ATTESTATION_INTERVAL_SECONDS`, open an aTLS connection to the Dev CVM's `/tdx_quote` endpoint, regenerate the complete Shade policy from the deployed compose, and run atlas-rs verification over `app_compose`, `expected_bootchain` (including the row's shared guest MRTD), `os_image_hash`, and the RTMR JCS replay. On success update `attestation_verified_at` and `image_measurement` / `rtmr3_digest`; emit `CVM_ATTESTATION_VERIFIED`. On any mismatch, set `error_reason = "ATTESTATION_DRIFT"`, emit `CVM_ATTESTATION_DRIFT`, and page the operator (§17.4) — the row is NOT auto-terminated; the Dev CVM's egress remains policed by the SC regardless. A refresh attempt failing with the connectivity code (`ATTESTATION_FETCH_FAILED`) is tolerated as transient (logged at `WARN`, retried next pass) while the last successful attestation is younger than `RECONCILER_ATTESTATION_INTERVAL_SECONDS + RECONCILER_ATTESTATION_UNREACHABLE_GRACE_SECONDS` (§12); past that, set `error_reason = "ATTESTATION_UNREACHABLE"` (never overwriting an existing `ATTESTATION_DRIFT` verdict or `SECURITY_CVM_REBIND_REQUIRED` marker, and only if `attestation_verified_at` still matches the claimed snapshot) and emit `CVM_ATTESTATION_UNREACHABLE` once per outage — the next successful refresh clears the mark.
...
| `SECURITY_CVM_ATLS_POLICY_UNAVAILABLE` | Dev CVM launch saga step `phala_deploy` when the entity SC has no complete stored policy for authenticated forwarder bootstrap | `component` |
...
| `SECURITY_CVM_ATTESTATION_UNREACHABLE` | reconciler attestation refresh past interval + grace (§9.2) | One row per outage. `after` carries `{error_reason: "ATTESTATION_UNREACHABLE", failure_code, source: "reconciler"}`. Never overwrites `ATTESTATION_DRIFT` or `SECURITY_CVM_REBIND_REQUIRED`.
...
| `CVM_ATTESTATION_UNREACHABLE` | reconciler attestation refresh past interval + grace (§9.2) | One row per outage. `after` carries `{error_reason: "ATTESTATION_UNREACHABLE", failure_code, source: "reconciler"}`. Never overwrites `ATTESTATION_DRIFT` or `SECURITY_CVM_REBIND_REQUIRED`.
...
| `PROFILE_POLICY_UPDATED` | `PATCH /profiles/{id}` (§3.4) when `policy` changed. |
| `PROFILE_SECRET_MINTED` | `POST /profiles/{id}/secrets/{injection_id}` and OAuth connect mint. | `after.injection_id`; never the secret value. |
| `PROFILE_MANAGED_SECRET_CONFIGURED` | `PUT /profiles/{id}/managed-secrets/{injection_id}` ([oauth-connections.md](oauth-connections.md)). | `after` carries injection_id, token_url, client_id; never the refresh token. |
| `PROFILE_MANAGED_SECRET_DELETED` | `DELETE /profiles/{id}/managed-secrets/{injection_id}`. | `after.injection_id`. |
| `PROFILE_SECRET_ROTATED` | managed-secret rotation success. | `after.injection_id`. |
| `PROFILE_SECRET_ROTATION_FAILED` | managed-secret rotation failure. | `after` carries injection_id and a sanitized error code. | `before.policy_sha256` and `after.policy_sha256` are recorded so the audit row size stays bounded; the full policies live on the `entity_profiles` row's history (or a separate version table — implementation detail). |
...
| `OAUTH_INTEGRATION_CONFIGURED` | `PUT /entities/{id}/oauth-integrations/{name}` ([oauth-connections.md](oauth-connections.md)). | Write-only client secret. |
| `OAUTH_INTEGRATION_DELETED` | `DELETE /entities/{id}/oauth-integrations/{name}`. | |
| `PROFILE_CONNECTION_LINK_CREATED` | `POST /profiles/{id}/connections`. | Connect-link creation; `after` carries integration name, never tokens. |
...
| `RECONCILER_ATTESTATION_UNREACHABLE_GRACE_SECONDS` | int 60..86400 | `1800` | Extra window past `RECONCILER_ATTESTATION_INTERVAL_SECONDS` during which failing attestation refreshes are treated as transient (§9.2 items 8–9). Once a RUNNING CVM's last successful attestation is older than interval + grace, the reconciler marks it `ATTESTATION_UNREACHABLE` and emits the matching audit event. |
| `OAUTH_TOKEN_URL_HOSTS` | comma-list of hosts | `auth.openai.com,slack.com` | Allowlist for Console-initiated OAuth token-endpoint requests ([oauth-connections.md](oauth-connections.md)). |
| `MANAGED_SECRET_ROTATION_GATE_SECONDS` | int ≥ 30 | `300` | Wall-clock gate between managed-secret rotation passes. |
| `MANAGED_SECRET_ROTATION_THRESHOLD_SECONDS` | int ≥ 60 | `432000` | Rotate when less than this much access-token life remains. |
| `MANAGED_SECRET_DEFAULT_TTL_SECONDS` | int ≥ 300 | `864000` | Fallback access-token TTL when the provider omits `expires_in` and JWT `exp`. |
