...
Updating the Security CVM (SC) image changes the SC's measured `app_compose`. Each Dev CVM's egress forwarder pins that `app_compose` through the complete aTLS policy it authenticated-fetches from the Console before its first upstream SC connection or successful CONNECT response and periodically thereafter. An old policy against a new-image SC produces `app_compose_hash_mismatch` and fail-closed egress. Historically, when the policy was fixed in launch material, recovering required a fleet-wide Dev CVM update.

The authenticated policy-bootstrap and refresh path in current Umbra Dev images removes that operational fleet churn: the full policy is not sent through Phala's launch env, the forwarder may listen for measurement compatibility but must fetch and strictly validate the complete policy before its first upstream SC connection or successful CONNECT response, and it periodically fetches updates from the same endpoint after the runtime CA poll accepts the current CA. Fetch or validation failure returns fail-closed `502`; blank, stub, dev, disabled, or incomplete delivered policies and validation bypasses are forbidden. A persisted `SECURITY_CVM_REBIND_REQUIRED` marker does not prove this runtime exists; the renamed build leaves that resource fail-closed for termination/decommission through the pre-Umbra control plane and replacement under Umbra. The fetched policy still comes from the Console, however, so it is not an independently signed release measurement. Until release-pipeline-signed SC policy material is verified locally, the Dev aTLS helper remains hardcoded to **disable the SC image/runtime policy check** and accepts any genuine SC TEE at the bound FQDN regardless of its app image.
