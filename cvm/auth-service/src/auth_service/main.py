...
# Minimum length for AUTH_TOKEN to be considered valid (Default: 32)
MIN_AUTH_TOKEN_LEN = os.environ.get("MIN_AUTH_TOKEN_LEN")
if MIN_AUTH_TOKEN_LEN is None:
    MIN_AUTH_TOKEN_LEN = 32
else:
    MIN_AUTH_TOKEN_LEN = int(MIN_AUTH_TOKEN_LEN)

AUTH_TOKEN_LEN = len(os.environ.get("AUTH_TOKEN", ""))
AUTH_TOKEN_HASH = (
    hash_token(os.environ.get("AUTH_TOKEN")) if AUTH_TOKEN_LEN >= MIN_AUTH_TOKEN_LEN else None
)
...
class AuthHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/auth":
            ...
            if token_match(token, AUTH_TOKEN_HASH):
...
def main():
    if not AUTH_TOKEN_HASH:
        logger.warning(
            "AUTH_TOKEN environment not set or too short (min is 32) - all auth requests will fail"
        )
